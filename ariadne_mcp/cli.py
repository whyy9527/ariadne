...
DEMO_REPO_URL = "https://github.com/spring-petclinic/spring-petclinic-microservices.git"
DEMO_HINT_DEFAULT = "owner"
DEMO_SERVICES = [
...
def _demo_cache_dir() -> str:
    base = os.environ.get("XDG_CACHE_HOME") or os.path.join(os.path.expanduser("~"), ".cache")
    return os.path.join(base, "ariadne-mcp", "demo")


def cmd_demo(args):
    """Zero-config end-to-end demo against the public spring-petclinic sample.

    Clones spring-petclinic-microservices into a cache dir (~/.cache/ariadne-mcp/demo),
    scans it, runs a query, prints the top cluster. No config file needed.
    """
    cache_dir = _demo_cache_dir()
    clone_dir = os.path.join(cache_dir, "spring-petclinic-microservices")
    config_path = os.path.join(cache_dir, "ariadne.config.json")
    db_path = os.path.join(cache_dir, "demo.db")

    os.makedirs(cache_dir, exist_ok=True)

    # 1. Clone (shallow) if missing, else reuse.
    if os.path.isdir(os.path.join(clone_dir, ".git")):
        print(f"==> Using cached clone at {clone_dir}")
    else:
        print(f"==> Cloning {DEMO_REPO_URL} (depth=1)")
        try:
            subprocess.run(
                ["git", "clone", "--depth", "1", DEMO_REPO_URL, clone_dir],
                check=True,
            )
            ...
            print(f"ERROR: git clone failed (exit {e.returncode}).", file=sys.stderr)
            sys.exit(1)

    # 2. Write config alongside the clone. Paths resolve relative to config file.
    demo_config = {
        "_comment": (
            "Auto-generated by `ariadne-mcp demo`. Edit or regenerate by re-running the command."
        ),
        "repos": [
            {"path": f"spring-petclinic-microservices/{svc}"} for svc in DEMO_SERVICES
        ],
    }
    ...
    summary = run_scan(config_path, db_path)
    print(f"==> Scan done: {summary['nodes']} nodes, {summary['duration_ms']} ms")

    # 4. Query and print top clusters.
    from ariadne_mcp.store.db import DB
    from ariadne_mcp.query.query import query, print_results

    hint = args.hint if args.hint else DEMO_HINT_DEFAULT
    db = DB(db_path)
    print(f"\nQuery: {hint}\n" + "=" * 50)
    results = query(db, hint, top_n=3)
    print_results(results)

    print(
        "\n"
        "Try more queries:\n"
        f"  ariadne-mcp --db {db_path} query \"vet\"\n"
        f"  ariadne-mcp --db {db_path} query \"visit\"\n"
        f"  ariadne-mcp --db {db_path} expand findOwner\n"
        "\n"
        "Wire into Claude Code / Cursor as an MCP server by running:\n"
        f"  ariadne-mcp install {config_path} <your-workspace-dir>\n"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    demo_parser = sub.add_parser(
        "demo",
        help="Zero-config end-to-end demo against spring-petclinic (clone + scan + query)",
    )
    demo_parser.add_argument(
        "hint",
        nargs="?",
        default=None,
        help="Business term to query (default: 'owner')",
    )
